...
def plot_merging_trajectory(buffers, buffer_idx, config):
    """Plot bird's-eye view and speed profiles."""
...
def make_merging_animation(
    buffers, buffer_idx, config, fps=40,
):
    """Create a GIF animation showing car blocks."""
    states = np.array(buffers["states"][0, :buffer_idx, :])
    dt = config["env_params"]["dt"]
    p_y_target = config["cost_fn_params"]["p_y_target"]
    car_l, car_w = 4.0, 1.8
    ...
    # Lane markings
    all_px = np.concatenate([frames[:, 0], frames[:, 4]])
    lane_x_lo = all_px.min() - 20
    lane_x_hi = all_px.max() + 20
    for y_lane, ls in [
        (p_y_target + 1.75, "-"),
        (p_y_target - 1.75, "-"),
        (p_y_target - 3.5 + 1.75, ":"),
    ...
    colors = {"ego": "dodgerblue", "IDM": "orange"}
    rects = {}
    labels = {}
    for name in colors:
        r = Rectangle(
            (0, 0), car_l, car_w,
            fc=colors[name], ec="black",
            lw=0.8, alpha=0.85,
        )
        ax.add_patch(r)
        rects[name] = r
        t = ax.text(
            0, 0, name, ha="center", va="center",
    ...
    def update(i):
        ...
        # IDM vehicle on target lane
        idm_px = s[4]
        idm_py = p_y_target
        rects["IDM"].set_xy(
            (idm_px - car_l / 2, idm_py - car_w / 2)
        )
        labels["IDM"].set_position((idm_px, idm_py))
        # Pan x-axis
        x_min = min(s[0], s[4])
...
def main(config, save_dir):
    ...
    # Ground truth parameters (all scalars)
    true_T = jnp.array(config["env_params"]["true_T"])
    true_b = jnp.array(config["env_params"]["true_b"])
    true_k_lat = jnp.array(
        config["env_params"]["true_k_lat"]
    )
    true_d0 = jnp.array(
        config["env_params"]["true_d0"]
    )
    true_params = {
        "model": {
            "T": true_T,
    ...
    for step in range(1, config["total_steps"] + 1):
        ...
        # Step environment
        (
            state, next_obs, rewards,
            terminated, truncated, info,
        ) = step_fn(state, episode_length, action)
        ...
        if (
            ...
            if step % config["eval_freq"] == 0:
                ...
                # Parameter diff from ground truth
                ...
                buf_states = buffers["states"][
                    0, :buffer_idx
                ]
                ego_py_buf = buf_states[:, 1]
                lane_error = float(
                    jnp.mean(
                        jnp.abs(ego_py_buf - p_y_target)
                    )
                )

                # Safety: distance to IDM vehicle
                ego_px = buf_states[:, 0]
                ego_py = buf_states[:, 1]
                d_idm = jnp.sqrt(
                    (ego_px - buf_states[:, 4]) ** 2
                    + (ego_py - p_y_target) ** 2
                )
                safety_dist = float(jnp.mean(d_idm))

                # Individual parameter errors
                learned_T = (
                    train_state.params["model"]["T"]
                )
                learned_b = (
                    train_state.params["model"]["b"]
                )
                learned_k_lat = (
                    train_state.params["model"]["k_lat"]
                )
                learned_d0 = (
                    train_state.params["model"]["d0"]
                )
                param_log = {
                    "eval/multi_step_loss": (
                        multi_step_loss
                    ...
                    "params/T_err": float(
                        jnp.abs(learned_T - true_T)
                    ),
                    "params/b_err": float(
                        jnp.abs(learned_b - true_b)
                    ),
                    "params/k_lat_err": float(
                        jnp.abs(
                            learned_k_lat - true_k_lat
                        )
                    ),
                    "params/d0_err": float(
