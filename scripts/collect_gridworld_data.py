...
def main(config):
    ...
    print(f"Collecting {num_episodes} episodes with random exploration...")
    print(f"Max episode length: {max_episode_length}")

    for ep in range(num_episodes):
        states, actions, rewards, ep_len, key = collect_episode_random(
            key, reset_fn, step_fn, get_obs_fn, max_episode_length
        )
        ...
        # Plot trajectory
        if (ep + 1) % plot_freq == 0:
            print(f"Episode {ep+1}/{num_episodes}: length={ep_len}")
            ...
            wandb.log({f"trajectory/ep_{ep+1}": wandb.Image(fig)})
            plt.close(fig)
