#!/usr/bin/env python3
"""
Simple script to visualize trained multi-agent blocker-goal-seeker policies.

Usage:
    python visualize_blockers.py --policy-path trained_policies/ippo-blocker-goal-seeker/policy_params.pkl
"""
...
# ============================================================================
# This config is for 3 agents (Agents 0-1 are Blockers, Agent 2 is Seeker)
...
# Option 2: Specify custom initial states
# Agents 0..N-2 are Blockers, Agent N-1 is the Seeker
CUSTOM_INITIAL_STATES = [
    jnp.array([0.0, 0.5, 0.0, 0.0]),    # Agent 0 (Blocker)
    jnp.array([0.0, -0.5, 0.0, 0.0]),   # Agent 1 (Blocker)
    jnp.array([-0.5, 0.0, 0.0, 0.0]),   # Agent 2 (Seeker)
]
...
def load_policy(policy_path, config):
    ...
    # Initialize environment (using imported init_env)
    ...
    policy, policy_state = init_policy(
        key, config, None, None, normalizer, norm_params
    )
...
def run_episode(policy, params, reset_fn, step_fn, get_obs_fn, config, seed=0):
    ...
    if CUSTOM_INITIAL_STATES is not None:
        if len(CUSTOM_INITIAL_STATES) != num_agents:
            print(f"Warning: CUSTOM_INITIAL_STATES has {len(CUSTOM_INITIAL_STATES)} "
                  f"entries, but config has {num_agents} agents. Ignoring custom states.")
        else:
    ...
    for i in range(num_agents):
        label = "Blocker" if i < num_agents - 1 else "Seeker"
        print(
            f"  Agent {i} ({label}): pos=({agent_states[i, 0]:.2f}, {agent_states[i, 1]:.2f}), "
    ...
    total_rewards = traj_rewards.sum(axis=0)
    for i in range(num_agents):
        label = "Blocker" if i < num_agents - 1 else "Seeker"
        print(f"Total rewards: Agent {i} ({label})={total_rewards[i]:.2f}")
    
    if terminated:
        print("Termination: Goal Reached (Seeker wins)")
    elif truncated:
        print("Termination: Max Steps (Blockers win)")
    else:
        print("Termination: Unknown")
...
def animate_trajectory(
    ...
    seeker_size = config["env_params"].get("seeker_size", 0.05) # Default size
    blocker_size = config["env_params"].get("blocker_size", 0.05) # Default size

    # Define colors for agents
    # Blockers (Agents 0..N-2) = Red
    # Seeker (Agent N-1) = Blue
    colors = ["red"] * (num_agents - 1) + ["blue"]
    dark_colors = ["darkred"] * (num_agents - 1) + ["darkblue"]
    agent_labels = [f"Blocker {i}" for i in range(num_agents - 1)] + ["Seeker"]
    path_labels = [f"Blocker {i} path" for i in range(num_agents - 1)] + ["Seeker path"]
    ...
    ax_main.set_title(f"Blocker-Goal-Seeker Trajectory (Seed: {seed})")
    ...
    # Draw goal (as a star marker, no radius)
    ...
    agent_markers = []
    for i in range(num_agents):
        radius = blocker_size if i < num_agents - 1 else seeker_size
        marker = Circle(
            (0, 0),
            radius,
            facecolor=colors[i],
    ...
    for i in range(num_agents):
        (line,) = ax_reward.plot(
            [], [], "-", color=colors[i], label=agent_labels[i], linewidth=2
        )
    ...
    def animate(frame):
        ...
        for i in range(num_agents):
            dist_to_goal = np.linalg.norm(agent_states[i][frame, :2] - goal)
            text_lines.append(f"Dist {agent_labels[i]}-Goal: {dist_to_goal:.2f}m")
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize trained multi-agent blocker-seeker policy"
    )
    parser.add_argument(
        "--policy-path",
        type=str,
        default="trained_policies/ippo-blocker-goal-seeker/policy_params.pkl",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="figures/blocker_animation.gif",
