#!/bin/bash
# LR sweep for ekf_batch trainer. Runs sequentially, one job at a time.
# Base: cheetah_ekf_batch.json with batch_size=1, H=3.
...
for lr in "${LRS[@]}"; do
    ...
    tmp_config="/tmp/ekf_lr_sweep_${lr}.json"
