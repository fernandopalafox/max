...
# ============================================================================
# PPO Policy Trainer Implementation
# ============================================================================
...
def create_ppo_policy_trainer(
    config: Any,
    policy: Policy,
    key: jax.Array,
) -> tuple[PolicyTrainer, PolicyTrainState]:
    """
    Creates a PPO policy trainer for actor-critic policies.
    """
    ...
    # Create separate optimizers for actor and critic (without gradient clipping here)
    ...
    def train_fn(
        ...
        Expected data dict:
        - 'states': (H, S) - states
        - 'actions': (H, A) - actions taken
        - 'rewards': (H,) - rewards received
        - 'dones': (H,) - done flags
        - 'log_pis_old': (H,) - log probabilities of actions (old policy)
        - 'values_old': (H,) - value estimates (old policy)
        ...
        advantages, value_targets = compute_gae(
            rewards, values_old, dones, last_value, ppo_gamma, ppo_lambda
        )
        ...
        def ppo_loss_fn(
            ...
            value_loss = (
                0.5
                * jnp.maximum(value_loss_unclipped, value_loss_clipped).mean()
            )
        ...
        def update_epoch(carry, _):
            """Run one full epoch through the shuffled buffer."""
            (
                actor_params,
                critic_params,
                actor_opt_state,
            ...
            log_pis_old_shuffled = log_pis_old[perm]
            advantages_shuffled = advantages[perm]
            value_targets_shuffled = value_targets[perm]
            values_old_shuffled = values_old[perm]
            ...
            def update_minibatch(carry, batch_data):
                ...
                # Unpack batch data (no dynamic indexing needed!)
                (
                    batch_states,
                    batch_actions,
                    batch_log_pis_old,
        ...
        new_opt_state = {"actor": actor_opt_state, "critic": critic_opt_state}
        new_train_state = train_state.replace(
            params=new_params,
...
def init_policy_trainer(
    ...
    if policy_trainer_type == "ppo":
        trainer, train_state = create_ppo_policy_trainer(config, policy, key)
        # Initialize optimizer state with actual policy params
        policy_trainer_params = config.get("policy_trainer_params", {})
        actor_lr = policy_trainer_params.get("actor_lr", 3e-4)
        critic_lr = policy_trainer_params.get("critic_lr", 1e-3)
        ...
        actor_opt_state = actor_optimizer.init(policy_params["actor"])
        critic_opt_state = critic_optimizer.init(policy_params["critic"])
